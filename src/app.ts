...
export default (opt: {
  ...
  function setCursorLocked(b: boolean): void {
    if (b) {
      try {
        const res =
          state.canvas.requestPointerLock() as unknown as Promise<void>;
        if (res.catch) {
  ...
  function isGamepadButtonPressed(
    ...
    return btn === undefined
      ...
      : setHasOrIncludes(state.mergedGamepadState.buttonState.pressed, btn);
  }

  function isGamepadButtonDown(btn?: GamepadButton | GamepadButton[]): boolean {
    return btn === undefined
  ...
  function isGamepadButtonReleased(
    ...
    return btn === undefined
      ...
      : setHasOrIncludes(state.mergedGamepadState.buttonState.released, btn);
  }
  ...
  function getButtonNameByKey(key: Key): string | undefined {
    for (const b in state.buttons) {
      ...
      if (bindings.includes(key)) {
        return Object.keys(state.buttons).find(
          (k) => state.buttons[k] === state.buttons[b]
        );
      }
    }
  }

  function getButtonByGamepadButton(btn: GamepadButton): string | undefined {
    for (const b in state.buttons) {
  ...
  function getButtonByMouseButton(btn: MouseButton): string | undefined {
    for (const b in state.buttons) {
  ...
  const onMouseDown = overload2(
    ...
    (mouse: MouseButton | MouseButton[], action: (m: MouseButton) => void) => {
      return state.events.on(
        "mouseDown",
  ...
  const onGamepadButtonDown = overload2(
    ...
    (btn: GamepadButton, action: (btn: GamepadButton) => void) => {
      return state.events.on(
        "gamepadButtonDown",
        (b) => isEqOrIncludes(btn, b) && action(b)
      );
  ...
  function registerGamepad(browserGamepad: Gamepad) {
    const gamepad = {
      ...
      isPressed: (btn: GamepadButton) => {
        return state.gamepadStates
          .get(browserGamepad.index)
          .buttonState.pressed.has(btn);
      },
      isDown: (btn: GamepadButton) => {
        return state.gamepadStates
          .get(browserGamepad.index)
          .buttonState.down.has(btn);
      },
      isReleased: (btn: GamepadButton) => {
        return state.gamepadStates
          .get(browserGamepad.index)
          .buttonState.released.has(btn);
      },
      getStick: (stick: GamepadStick) => {
        return state.gamepadStates
          .get(browserGamepad.index)
          .stickState.get(stick);
      },
    };
  ...
  function processGamepad() {
    ...
    for (const gamepad of state.gamepads) {
      ...
      const map =
        customMap[browserGamepad.id] ??
        GAMEPAD_MAP[browserGamepad.id] ??
        GAMEPAD_MAP["default"];
      ...
      for (let i = 0; i < browserGamepad.buttons.length; i++) {
        ...
        if (browserGamepadBtn.pressed) {
          ...
          if (gamepadState.buttonState.down.has(gamepadBtn)) {
            ...
            state.events.trigger("gamepadButtonRelease", gamepadBtn);
